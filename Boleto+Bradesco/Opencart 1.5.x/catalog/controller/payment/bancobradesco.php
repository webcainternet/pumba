...
class ControllerPaymentBancoBradesco extends Controller {
	protected function index() {
		...
		$this->data['url_boleto'] = $this->url->link('payment/bancobradesco/gerarboleto&pedido='.$this->session->data['order_id'].'');
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/bancobradesco.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/bancobradesco.tpl';
		} else {
			$this->template = 'default/template/payment/bancobradesco.tpl';
		}	
	...
	public function confirm() {
		...
		$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('bancobradesco_order_status_id'),$msg,true);
	}
}
?>